...
export type LiveTrainingChatRequest = {
  trainingPart: string;
...
function buildLiveTrainingChatPrompt(req: LiveTrainingChatRequest): string {
  return `
あなたはFITRAという筋トレAIコーチです。

### 現在の状況
- 部位: ${req.trainingPart || "未選択"}
- 強度: ${req.intensityMode}
- 種目: ${req.exerciseName || "未選択"}
- セット数: ${req.setNumber}
...
export async function getLiveTrainingChat(req: LiveTrainingChatRequest) {
  const prompt = buildLiveTrainingChatPrompt(req);
