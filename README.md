...
- Production URL：https://fitra-app.com
...
FITRAは、食事・トレーニング・生活の記録を個別に行い、それぞれの分析結果を統合Dashboardに集約するアプリです。  
以下では、主要画面ごとに実装内容と画面イメージを紹介します。
...
FITRAの中心となる画面です。  
食事・トレーニング・生活の分析結果を集約し、今日の身体状態を総合スコアとして確認できます。

総合スコアだけでなく、食事・トレーニング・生活それぞれのスコアも表示し、どの領域が強みで、どこを改善すべきかを見えるようにしています。

![FITRA Dashboard 1](./docs/images/dashboard_1.png)

直近の分析結果、スコア推移、分析履歴を確認できます。  
各領域の分析結果を日付ごとにまとめ、身体状態の変化を追えるようにしています。

![FITRA Dashboard 2](./docs/images/dashboard_2.png)

---

### 2. 食事管理

食事Dashboardでは、保存済みの食事ログ、直近スコア、平均スコア、記録数を確認できます。  
食事を記録するだけでなく、過去のスコア推移を振り返れるようにしています。

![FITRA Diet 1](./docs/images/diet_1.png)

食事入力画面では、食事内容の入力、画像アップロード、AIによる食事分析を行います。  
ユーザーが簡単に入力できるようにしつつ、AIが食事内容を読み取り、スコアやコメントとして返す構成にしています。

![FITRA Diet 2](./docs/images/diet_2.png)

---

### 3. トレーニング管理

トレーニングDashboardでは、種目ごとの重量推移、総ボリューム、直近で伸びた種目、AIコメントを確認できます。  
単に記録を残すだけでなく、成長傾向を見えるようにすることを重視しています。

![FITRA Training 1](./docs/images/training_1.png)

リアルタイム記録画面では、トレーニング中にスマホから重量・回数を記録できます。  
通常記録とは別に、トレーニング中の実際の利用シーンを想定して、セットごとのデータを即時に蓄積できるようにしました。

![FITRA Training 2](./docs/images/training_2.png)

---

### 4. 生活管理

生活スコア画面では、睡眠・疲労・ストレスから今日の回復状態を確認できます。  
トレーニングや食事だけでなく、生活状態も含めて身体状態を判断するための画面です。

![FITRA Life 1](./docs/images/life_1.png)

疲労分析画面では、疲労度や動けそうメーター、気になる項目を入力し、AIコメントを確認できます。  
主観的になりやすい疲労状態を、できるだけ振り返りやすい形で記録できるようにしています。

![FITRA Life 2](./docs/images/life_2.png)
...
HOME画面では、アプリの概要、GitHub / README導線、スコア根拠ページへの導線を配置しています。  
未ログインでもアプリの概要は確認でき、記録・分析・Dashboard利用にはログインが必要な構成にしています。
...
本番環境では、Vercel / Neon / Firebase Authentication を利用しています。  
ログイン、DB保存、Dashboard反映まで本番環境で確認しています。
...
FITRAでは、実装だけでなく、機能ごとの詳細設計書・テスト仕様書も作成しています。
...
合計で4Book、各11シート規模の設計資料を作成し、実装内容と仕様の対応関係を確認できるようにしています。

この資料作成を通じて、単に画面を作るだけでなく、状態管理、API、DB、例外処理、テスト観点まで整理しながら開発することを意識しました。

特に、FITRAでは食事・トレーニング・生活のデータ構造がそれぞれ異なるため、画面表示前に共通形式へ整形する設計や、保存処理・Dashboard反映・例外時の挙動を整理する必要がありました。

そのため、実装後に仕様を後追いで確認するのではなく、実装内容・状態遷移・API・DB・テスト観点をドキュメントとして整理し、改善時に参照できる形にしています。

### 設計資料の例

![FITRA Design Document](./docs/images/design_doc.png)
...
現在の主要機能である、認証、食事記録、トレーニング記録、生活スコア、統合Dashboard、本番デプロイは実装済みです。  
今後は、実際の利用を想定して、ユーザーごとのデータ管理精度や分析精度を高めていく予定です。
...
また、ローカル環境だけでなく、本番環境でもログイン、DB保存、Dashboard反映まで確認しています。
